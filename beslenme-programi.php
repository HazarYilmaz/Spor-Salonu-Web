...
<body>
    ...
    <tbody>
        <?php
        $beslenme_cek = $conn->prepare("SELECT * FROM beslenme_programlari_besinler ORDER BY id DESC LIMIT 1");
        $beslenme_cek->execute();
        $hareket = $beslenme_cek->fetch(PDO::FETCH_ASSOC);

        if ($hareket) {
            ?>
            <tr>
                <td>
                    <b><?= $hareket['ogun_adi']; ?></b>
                </td>
