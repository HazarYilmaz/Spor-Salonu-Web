<?php
    ...
    if (isset($_POST["kaydet"])) {
        ...
        if (validateTC($tc)) {
            ...
            $stmt = $db->prepare("SELECT * FROM personel_kayit WHERE tc = :tc OR email = :email OR tel = :tel");
            ...
            if (count($existingData) > 0) {
                ...
                foreach ($existingData as $data) {
                    if ($data['tc'] == $tc) {
                        ...
                        if (confirm('Bu Tc Zaten Kayıtlı !')) {
                            window.location.href = 'personelekle.html';
                        }
                        else
                        {
                            window.location.href = 'personelekle.html';
                ...
                try {
                    $stmt = $db->prepare("INSERT INTO personel_kayit (adi, soyadi, sifre, tc, email, tel, tarih, pozisyon, maas)
                                          VALUES (:adi, :soyadi, :sifre, :tc, :email, :tel, :tarih, :pozisyon, :maas)");
